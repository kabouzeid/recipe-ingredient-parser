...
pub fn parse(str: &str) -> IngredientInfo {
    let pairs = IngreedientParser::parse(Rule::ingredient_and_amount_and_unit, str).unwrap();
    ...
    for pair in pairs {
        match pair.as_rule() {
            Rule::amount => {
                ...
                amount = Some(ValueWithSpan { value: parse_amount(pair), span });
            },
            Rule::unit => {
                let span = extract_span(pair.as_span());
                unit = Some(ValueWithSpan { value: parse_unit(pair), span });
            },
            Rule::container_size => {
                ...
                container_amount = Some(ValueWithSpan { value: parse_amount(amount_pair), span: amount_span });

                let unit_pair = pairs.next().unwrap();
                let unit_span = extract_span(unit_pair.as_span());
                container_unit = Some(ValueWithSpan { value: parse_unit(unit_pair), span: unit_span });
            },
            Rule::ingredient => {
                let span = extract_span(pair.as_span());
                ingredient = Some(ValueWithSpan { value: pair.as_str().to_string(), span });
            },
            Rule::EOI => continue,
            _ => unreachable!(),
        }
    ...
    IngredientInfo { amount, unit, container_amount, container_unit, ingredient }
}
...
fn parse_constant(pair: pest::iterators::Pair<Rule>) -> Constant {
    ...
    match pair.as_rule() {
        ...
        Rule::word_digit => Constant::Fraction(super::dictionary_en::look_up_word_digit(pair.as_str()), 1),
        _ => unreachable!(),
    }
}
    ...
    fn test_parse() {
        {
            ...
            assert_eq!(amount.value, Amount::Constant { value: Constant::Fraction(3, 2) });
...

        {
            ...
            let amount = info.amount.unwrap();
            assert_eq!(amount.value, Amount::Range { value_from: Constant::Fraction(2,1), value_to: Constant::Fraction(3,1) });
            assert_eq!(amount.span.from, 0);
